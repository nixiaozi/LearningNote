...
namespace StackExchangeRedisTest
{
    ...
    public static class RedisHelper
    {
        ...
        private static readonly string _pwd = "123456";
        private static readonly int _store_db = 1;
        ...
        /// <summary>
        /// 从redis获取缓存数据，如果没有直接通过function创建并返回
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        /// <param name="db"></param>
        /// <returns></returns>
        public static T StringGet<T>(string key, Func<T> function, int expireMinutes = 600, RedisFolderEnum folder = RedisFolderEnum.Root, RedisDBEnum db = RedisDBEnum.Default)
        {
            if (KeyExists(key, folder, db))
            {
                return StringGet<T>(key, folder, db);
            }
            else
            {
                var item = function();
                StringSet<T>(key, item, expireMinutes, folder, db);
                return item;
            }
        }
        ...
        #region TTL
        public static void SetExpiationTime(string key, DateTime ExpiationTime, RedisFolderEnum folder = RedisFolderEnum.Root, RedisDBEnum db = RedisDBEnum.Default)
        {
            string fd = GetDescription(folder);

            Manager.GetDatabase(GetOperationDB(db)).StringSet(string.IsNullOrEmpty(fd) ? key : fd + ":" + key,
                Manager.GetDatabase(GetOperationDB(db)).StringGet(string.IsNullOrEmpty(fd) ? key : fd + ":" + key), (ExpiationTime - DateTime.Now));
        }
